        ...
        try {
            ...
            if (attempts < maxAttempts) {
                ...
                if (errorContainer && errorContent) {
                    ...
                    errorContent.textContent = 'JSZip 或 FileSaver 库加载失败。请确保 jszip.min.js 和 FileSaver.min.js 文件存在于扩展目录中。';
                }
...
function initializeApp() {
    ...
    function renderFileList(tree) {
        ...
        function renderNode(node, level = 0, container) {
            ...
            if (node.type === 'folder') {
                ...
                label.htmlFor = `file-${node.obj_token}`;
        ...
        try {
            ...
            chrome.tabs.sendMessage(tab.id, {
                ...
                if (chrome.runtime.lastError) {
                    showError('无法连接到页面，请确保在飞书页面中刷新后重试');
                    return;
                }

                if (response && response.success) {
                    if (response.tree && response.tree.length > 0) {
                        renderFileList(response.tree);
                        hideResults();
                    } else {
    ...
    // 从树形结构中收集所有文件
    function getAllFilesFromTree(nodes) {
        const files = [];
        nodes.forEach(node => {
            if (node.type === 'file') {
                files.push(node);
            } else if (node.type === 'folder' && node.children) {
                files.push(...getAllFilesFromTree(node.children));
            }
        });
        return files;
    }
